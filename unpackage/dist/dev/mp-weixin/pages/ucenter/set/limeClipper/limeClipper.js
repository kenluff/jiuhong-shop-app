...
const pages_ucenter_set_limeClipper_utils = require("./utils.js");
...
const _sfc_main = {
  ...
  methods: {
    ...
    imgComputeSize(width, height) {
      const { imageWidth, imageHeight } = pages_ucenter_set_limeClipper_utils.calcImageSize(width, height, this);
    ...
    imgMarginDetectionScale(scale) {
      if (!this.isLimitMove)
        return;
      const currentScale = pages_ucenter_set_limeClipper_utils.calcImageScale(this, scale);
      this.imgMarginDetectionPosition(currentScale);
    },
    imgMarginDetectionPosition(scale) {
      if (!this.isLimitMove)
        return;
      const { scale: currentScale, left, top } = pages_ucenter_set_limeClipper_utils.calcImageOffset(this, scale);
      this.setDiffData({
        imageLeft: left,
        imageTop: top,
        scale: currentScale
    ...
    clipTouchStart(event) {
      ...
      const corner = pages_ucenter_set_limeClipper_utils.determineDirection(clipX, clipY, clipWidth, clipHeight, currentX, currentY);
      this.moveDuring();
      if (!corner) {
        return;
      }
    ...
    clipTouchMove(event) {
      ...
      if (flagClipTouch && throttleFlag) {
        ...
        this.throttle();
        const clipData = pages_ucenter_set_limeClipper_utils.clipTouchMoveOfCalculate(this, event);
        if (clipData) {
    ...
    imageTouchStart(e) {
      ...
      if (e.touches.length === 1) {
        ...
        const hypotenuseLength = pages_ucenter_set_limeClipper_utils.calcPythagoreanTheorem(width, height);
        touchRelative = [
          {
            x: clientXForLeft - imageLeft,
            y: clientYForLeft - imageTop
    ...
    imageTouchMove(e) {
      ...
      this.moveDuring();
      if (e.touches.length === 1) {
        const { left: imageLeft, top: imageTop } = pages_ucenter_set_limeClipper_utils.imageTouchMoveOfCalcOffset(this, clientXForLeft, clientYForLeft);
        this.setDiffData({
          imageLeft,
          imageTop
        });
        ...
        const clientXForRight = e.touches[1].clientX;
        const clientYForRight = e.touches[1].clientY;
        let width = Math.abs(clientXForLeft - clientXForRight), height = Math.abs(clientYForLeft - clientYForRight), hypotenuse = pages_ucenter_set_limeClipper_utils.calcPythagoreanTheorem(width, height), scale = this.scale * (hypotenuse / this.hypotenuseLength);
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-b648bbb0"], ["__file", "E:/uniapp/\u4E5D\u9E3F\u5546\u57CE\u5BA2\u6237\u7AEFuniapp\u7248/pages/ucenter/set/limeClipper/limeClipper.vue"]]);
